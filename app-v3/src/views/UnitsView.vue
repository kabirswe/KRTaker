  ...
  <div>
    <div class="page-head">
      ...
      <div class="head-actions" style="display:flex;gap:8px;flex-wrap:wrap;align-items:center">
        <input v-model="query" placeholder="Search unit, property, tenant…" style="padding:9px 13px;border:1px solid var(--border);border-radius:10px;background:var(--bg-alt);font-family:inherit;font-size:13px;color:var(--text);outline:none;width:220px">
      ...
      <div v-for="u in filtered" :key="u.id" class="panel chip" style="cursor:pointer;overflow:hidden;display:flex;flex-direction:column" @click="openDetail(u)">
        ...
        <div style="padding:13px 15px;flex:1;display:flex;flex-direction:column;gap:9px">
          ...
          <div style="display:flex;gap:13px;font-size:12px">
            <span class="c-sub" title="Area">📐 {{ (u.sqft || 0).toLocaleString('en-IN') }} sqft</span>
            <span class="c-sub" title="Rent">💵 {{ money(u.rent) }}/mo</span>
            <span v-if="u.beds && u.beds !== '0'" class="c-sub" title="Beds">🛏️ {{ u.beds }}</span>
            <span v-if="u.baths && u.baths !== '0'" class="c-sub" title="Baths">🛁 {{ u.baths }}</span>
            <span v-if="String(u.furnished) === '1'" class="c-sub" title="Furnished">🛋️</span>
          </div>
    ...
    <div v-else class="panel" style="padding:40px;text-align:center;color:var(--text-mute)">No units found{{ query ? ' for “' + query + '”' : '' }}.</div>
          ...
          <table v-else-if="tab === 'invoices'" class="kr" style="width:100%">
            ...
            <tbody>
              <tr v-for="i in selInvoices" :key="i.id">
                <td style="font-weight:700">{{ i.id }}</td><td>{{ i.m || '—' }}</td><td>{{ money(i.gross) }}</td><td>{{ money(i.tds) }}</td><td style="font-weight:700">{{ money(i.net) }}</td>
          ...
          <table v-else-if="tab === 'tickets'" class="kr" style="width:100%">
            ...
            <tbody>
              <tr v-for="t in selTickets" :key="t.id">
                <td style="font-weight:700">{{ t.id }}</td><td>{{ t.desc }}</td><td>{{ t.reported || '—' }}</td><td>{{ t.liab || '—' }}</td><td>{{ money(t.cost) }}</td>
    ...
    <template v-if="modal">
      ...
      <div style="position:fixed;top:50%;left:50%;transform:translate(-50%,-50%);width:min(500px,94vw);background:var(--card);border-radius:16px;z-index:71;box-shadow:0 24px 70px rgba(0,0,0,.3);max-height:90vh;overflow-y:auto">
        ...
        <div style="padding:18px 22px;display:grid;grid-template-columns:1fr 1fr;gap:13px">
          ...
          <div>
            <label style="font-size:11.5px;font-weight:800;color:var(--text-mute);text-transform:uppercase;letter-spacing:.3px">Floor</label>
          ...
          <div>
            <label style="font-size:11.5px;font-weight:800;color:var(--text-mute);text-transform:uppercase;letter-spacing:.3px">Status</label>
          ...
          <div>
            <label style="font-size:11.5px;font-weight:800;color:var(--text-mute);text-transform:uppercase;letter-spacing:.3px">Beds</label>
          ...
          <div>
            <label style="font-size:11.5px;font-weight:800;color:var(--text-mute);text-transform:uppercase;letter-spacing:.3px">Baths</label>
          ...
          <div>
            <label style="font-size:11.5px;font-weight:800;color:var(--text-mute);text-transform:uppercase;letter-spacing:.3px">Furnished</label>
