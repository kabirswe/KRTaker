...
async function submitTicket() {
  composeErr.value = ''
  if (!form.value.subject.trim()) { composeErr.value = t('Subject is required.'); return }
  busy.value = true
  try {
...
async function setStatus(s) {
  ...
  try {
    ...
    window.__krToast?.('✅ ' + t('Status → {s}').replace('{s}', s))
  } finally { threadBusy.value = false }
}
async function setPrio(p) {
  ...
  try {
    ...
    window.__krToast?.('✅ ' + t('Priority → {p}').replace('{p}', p))
  } finally { threadBusy.value = false }
}
  ...
  <div>
    <div class="page-head">
      <div>
        ...
        <div class="sub">{{ supportAll.length }} {{ lang === 'bn' ? 'টি টিকেট' : 'tickets' }} · {{ kpis[1]?.value || 0 }} {{ lang === 'bn' ? 'খোলা' : 'open' }} · {{ lang === 'bn' ? 'লাইভ এপিআই থেকে' : 'live from API' }}</div>
      </div>
      ...
      <CompactFilters>
        ...
        <div style="display:flex;border:1px solid var(--border);border-radius:10px;overflow:hidden">
          <button @click="viewMode = 'grid'" :style="viewMode === 'grid' ? 'background:var(--primary);color:#fff' : 'background:var(--bg-alt);color:var(--text-mute)'" style="padding:8px 12px;border:none;font-size:12.5px;font-weight:800;cursor:pointer">▦ {{ lang === 'bn' ? 'গ্রিড' : 'Grid' }}</button>
          <button @click="viewMode = 'list'" :style="viewMode === 'list' ? 'background:var(--primary);color:#fff' : 'background:var(--bg-alt);color:var(--text-mute)'" style="padding:8px 12px;border:none;font-size:12.5px;font-weight:800;cursor:pointer">☰ {{ lang === 'bn' ? 'তালিকা' : 'List' }}</button>
        </div>
    ...
    <div v-if="catOptions.length > 1" style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:14px">
      <button @click="catFilter = ''" class="badge b-gray"
        ...
        style="padding:7px 13px;font-size:12.5px;font-weight:800;cursor:pointer">🗂 {{ lang === 'bn' ? 'সব' : 'All' }}</button>
      <button v-for="c in catOptions" :key="c" @click="catFilter = (catFilter === c ? '' : c)" class="badge b-gray"
        ...
        {{ catIco(c) }} {{ t(c) }} <span style="font-size:11px;opacity:.75">{{ supportAll.filter(t => (t.cat || 'General') === c).length }}</span>
      </button>
    </div>
    ...
    <div v-if="!filtered.length" class="panel" style="padding:40px;text-align:center;color:var(--text-mute)">{{ t('No tickets found') }}{{ query ? ' — "' + query + '"' : '' }}.</div>
    ...
    <template v-if="showCompose">
      ...
      <div style="position:fixed;top:50%;left:50%;transform:translate(-50%,-50%);width:min(520px,94vw);background:var(--card);border-radius:18px;z-index:61;box-shadow:0 24px 80px rgba(0,0,0,.35);overflow:hidden">
        ...
        <div style="padding:18px 20px 20px">
          <div style="font-size:12px;font-weight:800;color:var(--text-mute);margin-bottom:6px">{{ t('Subject') }} *</div>
          ...
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:14px">
            <div>
              ...
              <select v-model="form.cat" style="width:100%;padding:10px;border-radius:10px;border:1px solid var(--border);background:var(--bg);color:var(--text);font-size:13.5px;outline:none">
                <option v-for="c in CATEGORIES" :key="c" :value="c">{{ catIco(c) }} {{ t(c) }}</option>
              </select>
            </div>
            <div>
          ...
          <textarea v-model="form.body" rows="4" :placeholder="t('Describe the issue — what happened, when, and any error you saw…')" style="width:100%;padding:11px 14px;border-radius:10px;border:1px solid var(--border);background:var(--bg);color:var(--text);font-size:13.5px;outline:none;resize:vertical;font-family:inherit"></textarea>
          ...
          <div style="display:flex;gap:10px;justify-content:flex-end;margin-top:16px">
            ...
            <button @click="submitTicket" :disabled="busy" style="padding:10px 20px;font-size:13px;font-weight:800;border:none;border-radius:10px;background:var(--primary);color:#fff;cursor:pointer">{{ busy ? t('Opening…') : t('Open ticket') }}</button>
          </div>
        </div>
      </div>
    </template>
      ...
      <div style="position:fixed;top:0;right:0;bottom:0;width:min(560px,94vw);background:var(--card);z-index:61;box-shadow:-18px 0 50px rgba(0,0,0,.18);display:flex;flex-direction:column;overflow:hidden">
        ...
        <div style="padding:18px 20px 0;overflow-y:auto;flex:1">
          ...
          <div style="display:flex;gap:8px;margin:14px 0 8px">
            <input v-model="reply" @keyup.enter="sendReply" :placeholder="t('Write a reply…')" :disabled="threadBusy"
