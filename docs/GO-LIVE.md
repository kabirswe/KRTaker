...
- [x] Review terms.html / privacy.html / contact.html for: entity name, address, refund policy, DPA-2023 disclosures, dispute/complaint channel, data-subject request procedure. (Reviewed 2026-08-16. **FOUND + FIXED a shipped regression**: commit `4458139` (security-page batch) had committed a stale working tree — 28 landing pages lost ~3,100 lines vs V2.43: terms §3 refund policy (14-day refund / cancel-anytime) was reverted to "fees non-refundable", GA4 removed from pages, `nav.docs`/`footer.docs` i18n keys deleted, css cache-buster + manifest lost. Rebuilt: restored all web/ files to V2.43 (`528dde0`) baseline and re-applied ONLY the intended additions (security.html + footer 🔒 Security link on 28 pages + `footer.security` i18n EN/BN + terms §6 / privacy §7 body cross-links). Deployed 36/36 via UAPI + i18n dict; **live-verified**: terms refund policy back, GA4 back (4 refs), security footer + page 200, i18n keys present. **Current legal-pages state**: refund/cancel policy ✓ (§3), multi-tenant isolation + retention + export + privacy@krtaker.com 30-day response ✓ (privacy.html), TLS/AES-256/backups ✓ (security.html + privacy §7). **Still blocked on §0.1**: legal entity name (BITSCOL vs KRTaker) + full street address + explicit DPA-2023 registration statement + dedicated dispute-resolution clause — need the entity decision before finalizing.)
- [x] Add a "Security" page or badge section (SSl, data localization, backups) — buyers in BD ask. (2026-08-16: `web/security.html` trust page live — TLS 1.2+/AES-256 at rest, data localization in BD, multi-tenant isolation, 2FA + session controls, daily backups + restore drill, AI processing audit, no card storage, responsible disclosure (security@krtaker.com). Footer 🔒 Security link on all 28 pages + homepage; cross-linked from terms §6 + privacy §7.)
